...
class Controller
{
    ...
    public function __construct($json)
    {
        ...
        $this->Init = $this->InitialRelationData($init_data);
    }
    public function InitialRelationData($Init)
    {
        if (!array_key_exists('Relation',  $Init))
            return $Init;
        foreach ($Init["Relation"] as $key => $value) {
            if (!array_key_exists('file_relation',  $value))
                continue;
            $ins = json_decode(file_get_contents(app_path($value["file_relation"]["file"])), true);
            $Init["Relation"][$key]["resource"] = $ins;
        }
        return $Init;
    }
    ...
    public function create_master_validation($master, $resource)
    {
        $va = [];
        $resource
        foreach ($data as $key => $value) {
