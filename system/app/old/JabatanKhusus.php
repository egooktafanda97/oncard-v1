...
class JabatanKhusus extends Model
{
    use HasFactory;
    ...
    public function __construct(array $attributes = [])
    {
        ...
        */
        $instace = new ControllerSource("JabatanKhusus");
    ...
    public function initial_relation($instace)
    {
        if ($instace->getRelation()) {
            foreach ($instace->getRelation() as $model_name) {
                ...
                if ($RelationShip) {
                    $this->addDynamicRelation($model_name, function (JabatanKhusus $this_model) use ($RelationShip) {
